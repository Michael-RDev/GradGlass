...
class DiffResult:
    ...
    def to_dict(self, include_delta=False):
        d = {
            ...
            "frob_norm": round(self.frob_norm, 6),
            "cos_sim": round(self.cos_sim, 6),
            "percent_changed": round(self.percent_changed, 4),
...
class FullDiffResult:
    ...
    def to_dict(self, include_deltas=False):
        return {
            ...
            "step_a": self.step_a,
            "step_b": self.step_b,
            "layers": [lr.to_dict(include_delta=include_deltas) for lr in self.layers],
...
def activation_diff(acts_a, acts_b, layer_name=""):
    ...
    return {
        ...
        "sparsity_delta": sparsity_b - sparsity_a,
...
def architecture_diff(arch_a, arch_b):
    layers_a = {layer["id"]: layer for layer in arch_a.get("layers", [])}
    layers_b = {layer["id"]: layer for layer in arch_b.get("layers", [])}
    ...
    return {
        "added_layers": [layers_b[lid] for lid in added],
        "removed_layers": [layers_a[lid] for lid in removed],
        "type_changed": type_changed,
        "shape_changed": shape_changed,
        "param_changed": param_changed,
        "added_edges": [list(e) for e in edges_b - edges_a],
        "removed_edges": [list(e) for e in edges_a - edges_b],
        "is_identical": len(added) == 0 and len(removed) == 0 and (len(type_changed) == 0),
    }
...
def top_k_deltas(delta, k=10):
    ...
    for idx in top_indices:
        ...
        results.append(
            {
                "index": list((int(i) for i in multi_idx)),
                "value": float(delta.flatten()[idx]),
                "abs_value": float(flat[idx]),
            }
        )
    return results
