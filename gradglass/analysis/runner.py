...
class AnalysisRunner:
        ...
        for test_id in test_ids:
            ...
            if registered is None:
                results.append(
                    TestResult(
                        id=test_id,
                        title=f"Unknown test: {test_id}",
                        status=TestStatus.SKIP,
                        severity=TestSeverity.LOW,
                        category=TestCategory.ARTIFACT,
                        details={"error": "Test not found in registry"},
                    )
                )
                continue
            start = time.time()
            try:
                ...
                result = TestResult(
                    ...
                    recommendation="Test crashed. Check artifacts integrity.",
    ...
    def generate_summary_sections(self, ctx=None):
        ...
        if ctx.has_checkpoints:
            ...
            for ck in ctx.checkpoints_meta:
                ckpt_summary["checkpoints"].append(
                    {
                        "step": ck["step"],
                        "tag": ck.get("tag"),
                        "num_params": ck.get("num_params", 0),
            ...
            if len(ctx.checkpoint_steps()) >= 2:
                try:
                    ...
                    ckpt_summary["diff"] = {
                        "step_a": steps[0],
                        "step_b": steps[-1],
                        "layers_compared": diff_result.summary["total_layers"],
                        "severity_counts": {
                            "low": diff_result.summary.get("low", 0),
                            "medium": diff_result.summary.get("medium", 0),
                            "high": diff_result.summary.get("high", 0),
                            "critical": diff_result.summary.get("critical", 0),
                        },
        ...
        if ctx.has_grad_summaries:
            ...
            grad_summary = {
                "has_data": True,
                "layers_tracked": len(analysis),
                "healthy": len(healthy),
                "flagged": len(flagged),
                "flagged_layers": [
                    {"layer": a["layer"], "flags": a["flags"], "grad_norm": round(a["grad_norm"], 6)} for a in flagged
                ],
            }
    ...
    def render_text(self, sections, test_results):
        ...
        for c in ckpt.get("checkpoints", []):
            ...
            lines.append(
                f"    step={c['step']}{tag}  params={c.get('num_params', 0):,}  size={c.get('size_mb', '?')} MB"
            )
        diff = ckpt.get("diff")
        if diff:
            ...
            if top:
                ...
                for lr in top:
                    lines.append(
                        f"    {lr['layer']:<35} frob={lr['frob_norm']:.4f}  cos={lr['cos_sim']:.4f}  [{lr['severity']}]"
                    )
