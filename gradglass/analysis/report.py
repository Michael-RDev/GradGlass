...
class PostRunReport:
    ...
    def to_dict(self):
        return {
            ...
            "checkpoint_diff_summary": self.checkpoint_diff_summary,
            "gradient_flow_analysis": self.gradient_flow_analysis,
            "training_metrics_summary": self.training_metrics_summary,
            "artifact_store_summary": self.artifact_store_summary,
            "tests": self.tests,
        }
    ...
    def from_file(cls, run_dir):
        ...
        return cls(
            ...
            checkpoint_diff_summary=data.get("checkpoint_diff_summary", {}),
            gradient_flow_analysis=data.get("gradient_flow_analysis", {}),
            training_metrics_summary=data.get("training_metrics_summary", {}),
            artifact_store_summary=data.get("artifact_store_summary", {}),
    ...
    def generate(cls, run_id, store, run_dir, tests="all", save=True, print_summary=True):
        ...
        tests_dict = {
            ...
            "results": [t.to_dict() for t in test_results],
        }
        summary_text = runner.render_text(sections, test_results)
        report = cls(
            run_id=run_id,
            generated_at=time.strftime("%Y-%m-%d %H:%M:%S"),
            checkpoint_diff_summary=sections.get("checkpoint_diff_summary", {}),
            gradient_flow_analysis=sections.get("gradient_flow_analysis", {}),
            training_metrics_summary=sections.get("training_metrics_summary", {}),
            artifact_store_summary=sections.get("artifact_store_summary", {}),
            tests=tests_dict,
            summary_text=summary_text,
        )
        if save:
